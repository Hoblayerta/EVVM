...
if [[ $deployNow == "y" || $deployNow == "Y" ]]; then
    ...
    echo "  fuji   - Avalanche Fuji Testnet"
    echo "  custom - Custom RPC URL"
    echo ""

    while true; do
        read -p "$(echo -e "Select network (eth/arb/fuji/custom) ${GRAY}[eth]${NC}: ")" network
        network=${network:-"eth"}

        if [[ $network == "eth" || $network == "arb" || $network == "fuji" || $network == "custom" ]]; then
            break
        else
            echo -e "${RED}Error: Invalid network. Use 'eth', 'arb', 'fuji', or 'custom'${NC}"
        fi
    done
    ...
    echo -e "  ${YELLOW}For predefined networks: make deployTestnet NETWORK=<eth|arb|fuji>${NC}"
